// dataset: graphData
var graphData;
d3.json("/montreal_bikes_ussage/data/chart_data_MH.json", function(data) {
    graphData = data;
    var graphData2;
    d3.json("/montreal_bikes_ussage/data/chart_data2_MH.json", function(data) {
        graphData2 = data;
        ...
        for (var i = 0; i < 24; i++) {
            for (var j = 0; j < 6; j++) {
                a=graphData[i][j+4]
                b=graphData2[i][j+4]/60
                data[j+6*i] = {month: j +1, hour: i, value:graphData[i][j+4], mean_duration:graphData2[i][j+4]/60}
            }
        }
        ...
        var margin = { top: 50, right: 0, bottom: 100, left: 30 },
            ...
            months = ["Abril", "Mayo", "Junio", "Julio", "Agosto", "Semptiembre"],//, "Octubre","Noviembre"],
        ...
        cards.enter().append("rect")
            ...
            .attr("y", function(d) { return (d.month - 1) * gridSize + padding_top; })
            .attr("rx", 4)
            .attr("ry", 4)
            ...
            function(d,i)
            {
                d3.select('.tooltiptext')
                    ...
                    .text(function(d){
                        if(info == 'value')
                            return d[info] + ' viajes';
                        else
                            return d[info].toFixed(1) + ' minutos';
                    })

                d3.select('.tooltiprect')
                    .datum(d)
                    .attr('x',function(d){return (d.hour) * gridSize;})
                    .attr('y',function(d){return (d.month - 1) * gridSize + padding_top - 20; })
                    .attr('visibility', 'visible')
            })
            .on("mouseout",
                function(d,i)
